...
public class Server {
	...
	public synchronized void analyzeString(ClientHandler handler, String msg) {
		...
		if (input[0].equals(Protocol.CLIENT_CORE_JOIN)) {
			...
			if (!(handler.getGame().getBag().tileBag.containsValue(1)) || !(handler.getGame().getBag().tileBag.containsValue(2)) || !(handler.getGame().getBag().tileBag.containsValue(3))  || (!hand.contains(tile))){
