...
public class HumanPlayer extends Player {
	...
	public Map<String, Tile> determineMove(Game game) {
		...
		try {
			input = new BufferedReader(new InputStreamReader(System.in)).readLine().trim().toUpperCase();
		} catch (IOException e) {
		...
		if ((input.startsWith("MOVE") && input.length() > 9 && cantSwap.equals("empty"))) {
			...
			Tile tile = getHand().get(tileSelection);
			if (!bag.tileBag.containsValue(1) && !bag.tileBag.containsValue(2) && !bag.tileBag.containsValue(3)
					|| (!hand.contains(tile))) {
				...
				cantSwap = "swapped";
				/*
				 * while(!input.startsWith("DONE")){ hand = getHand(); bag =
				 * game.getBag(); tileSwap = input.replace("SWAP ", "");
				 * tileArray = tileSwap.split(Protocol.MESSAGESEPERATOR);
				 * tileSelection = Integer.parseInt(tileArray[0]) - 1; tile =
				 * getHand().get(tileSelection); if
				 * (!bag.tileBag.containsValue(1) &&
				 * !bag.tileBag.containsValue(2) &&
				 * !bag.tileBag.containsValue(3) || (!hand.contains(tile))){
				 * System.out.println(Protocol.SERVER_CORE_SWAP_DENIED);
				 * moveMap.putAll(this.determineMove(game)); } else {
				 * moveMap.put("SWAP " + moveMap.size(), tile);
				 * System.out.println(Protocol.SERVER_CORE_SWAP_ACCEPTED);
				 * cantSwap = "swapped"; } }
				 */
		...
		public void run() {
			...
			try {
				...
				if (!done) {
					...
					// System.out.println("Sadly we can't help you here, our AI
					// is probably worse than you are" );
					done = true;
				}
